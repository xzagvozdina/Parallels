...
int main()
{
	...
	//Последовательное разложение(OMP)
	start_time = clock();
	lup = LU_omp(a, n);
	end_time = clock();
...
void fullGauss(int n, double **M, double *r)//с полным выбором ГИ
{
	...
	//начальная инициализация массивов A и f из поступивших переменных **M и *r
	for (int i = 0; i < n; i++)
	{
		A[i] = new double[n];
		for (int j = 0; j < n; j++)
	...
	//ЗАКОММЕНТИТЬ В СЛУЧАЕ ВКЛЮЧЕНИЯ СОРТИРОВКИ!!!
	for (int i = 0; i < n; i++)
		r[i] = f[i];
